...
def get_benchmark_data2(n, rho, r, betas, gammas):
    '''
    ...
    W = np.random.poisson(size=(n, 2), lam=2)
    X = r * W + vs
    W = np.hstack((W ** 2, (W[:, 0] * W[:, 1]).reshape((n, 1)), W))
    W_w_const = np.hstack((np.ones((n, 1)), W))
    X_w_const = np.hstack((np.ones((n, 1)), X))
...
def get_data(n, rho, r, betas, gammas):

    errors = np.random.multivariate_normal(mean=np.zeros(4), cov=rho, size=n)
    us, eps = errors[:, :-1], errors[:, -1]
    vs = np.random.normal(size=(n, 2), loc=0, scale=(4 * np.sqrt(1 - r ** 2)))
    
    W = np.random.poisson(size=(n, 2), lam=3)
    X = r * W + vs 
    W_w_const = np.hstack((np.ones((n, 1)), W))
    X_w_const = np.hstack((np.ones((n, 1)), X))
...
def run_simulations(n, rho, r, betas, gammas, n_simulations=1000):
    ...
    for i in tqdm(range(n_simulations)):
        ...
        ## Лес с полиномами
        forest = RandomForestClassifier(n_estimators=10, max_depth=3, random_state=999)
    ...
    results = pd.DataFrame([ols_res_metrics, 
                            ...
                            forest_res_metrics], columns=cols)
    
    return results[cols_order_show]


def run_simulations2(n, rho, r, betas, gammas, n_simulations=1000):
    ...
    for i in tqdm(range(n_simulations)):
        ...
        df = get_data(n, rho, r, betas, gammas)
        df_no_nans = df.dropna()
        ...
        ## Бустинг с полиномами
        boosting = CatBoostClassifier(iterations=max(10, n // 1000 * 10), max_depth=3, verbose=0)
        boosting_results.append(get_results_ml_model(boosting, df, W, z))
        
        ## Лес с полиномами
        forest = RandomForestClassifier(n_estimators=100, max_depth=3, random_state=999)
        forest_results.append(get_results_ml_model(forest, df, W, z))
...
def run_simulations3(n, rho, r, betas, gammas, n_simulations=1000):
    ...
    for i in tqdm(range(n_simulations)):
        ...
        df = get_benchmark_data2(n, rho, r, betas, gammas)
        df_no_nans = df.dropna()
